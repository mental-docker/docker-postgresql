...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/mental/postgresql) and is the recommended method of installation.

> **Note**: Builds are also available on [Quay.io](https://quay.io/repository/mental/postgresql)

```bash
docker pull mental/postgresql:9.6-2
```

Alternatively you can build the image yourself.

```bash
docker build -t mental/postgresql github.com/mental-docker/postgresql
```
...
```bash
docker run --name postgresql -itd --restart always \
  mental/postgresql:9.6-2 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  mental/postgresql:9.6-2 -c logging_collector=on
```
