  ...
  if [[ -n ${DB_USER} ]]; then
    case $REPLICATION_MODE in
      ...
      *)
        ...
        IFS=',' read -ra PASSLIST <<< "$DB_PASS"
        userindex=0
        for userdb in $(awk -F',' '{for (i = 1 ; i <= NF ; i++) print $i}' <<< "${DB_USER}"); do
          echo "Creating database user: ${userdb}"
          if [[ -z $(psql -U ${PG_USER} -Atc "SELECT 1 FROM pg_catalog.pg_user WHERE usename = '${userdb}'";) ]]; then
            psql -U ${PG_USER} -c "CREATE ROLE \"${userdb}\" with LOGIN CREATEDB PASSWORD '${PASSLIST[${userindex}]}';" >/dev/null
          fi
          ((userindex+=1))
          echo "${userindex}. user created."
        done
        ;;
    esac
  fi
